...
def adsorp_equilibrium(yA,yB,P,norm,param):
  #for O2, N2
...
def init(param,norm):
  ...
  cA=C*state.yA[0]
  cB=C*state.yB[0]
  a,b=adsorp_equilibrium(state.yA,state.yB,state.P, norm,param)
...
def rate_of_change_vel(state,vin=None,vout=None,
  ...
  if param.adsorb:
    ...
    xAs,xBs=adsorp_equilibrium(state.yA,state.yB, state.P, norm, param)
...
###                                  method='Radau')
  ...
  data.data1.xAs,data.data1.xBs=adsorp_equilibrium(data.data1.yA,data.data1.yB,data.data1.P, norm, param)
  data.data2.xAs,data.data2.xBs=adsorp_equilibrium(data.data2.yA,data.data2.yB,data.data2.P, norm, param)

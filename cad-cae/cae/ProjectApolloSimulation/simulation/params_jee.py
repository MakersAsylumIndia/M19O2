...
def create_param(mods, print=print):
  ...
  param.D=2*2.5/100     #Diameter of cylinder [m]
  #param.L=.33       #Length of cylinder (m)  - 13"
  param.L=0.50       #Length of cylinder (m) 
  ...
  #flow params
  param.input_orifice=2.20     # dia mm - pressure feed
  param.output_orifice=0.80    # dia mm - to output tank
  #we use an orifice that will provide 5LPM at 3 bar abs product tank
  param.product_orifice=0.25   # dia mm - output from product tank to patient
  #try a smaller for testing
  #param.product_orifice=0.3   # dia mm - output from product tank to patient
  param.blowdown_orifice=4.00     # dia mm
  param.vent_orifice=1.8        # dia mm
  param.feed_pressure=4.0        #pressure in bar (1e5 Pa), absolute
  ...
  param.rho_pellet=1.170*1000  # [kg/m3]
  ...
  in_flow_m3_per_min=orifice.orifice_flow2(param.feed_pressure*1e5/1000,100,param.input_orifice,T=param.Ta)
  param.initial_in_flow_rate_lpm=in_flow_m3_per_min*1000
  in_flow=in_flow_m3_per_min/60
